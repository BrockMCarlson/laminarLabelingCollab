...
out.CSD = D_CSD_BASIC(LFP, 'cndt', cndt, 'spc', spc);

if exist('lfp_stream', 'var')
    [out.PSD, out.PSD_NORM, out.PSD_F]                          = D_PSD_BASIC(lfp_stream);
    out.CORRE                                                   = D_LFPCORR_BASIC(lfp_stream, win, true);
else
    [out.PSD, out.PSD_NORM, out.PSD_F]                          = D_PSD_BASIC(LFP);
    out.CORRE                                                   = D_LFPCORR_BASIC(LFP, win, true);
end
...
if do_plot
    ...
    ax1 = subplot(1, 3, 1);
    P_CSD_BASIC(nanmean(out.CSD(2:end-1, :, :),3), TV, tlim, f_h, ax1)
    
    ax2 = subplot(1, 3, 2);
    P_PSD_BASIC(out.PSD_NORM, out.PSD_F, f_h, ax2)
    
    ax3 = subplot(1, 3, 3);
    P_CORRE_BASIC(out.CORRE, f_h, ax3)
