...
for h = 1:1
    ...
    for i = 1:size(ns2BRFSfiles,2)
        ...
        % Select trials
            ...
            EVP = mean(SDF800,3); % EVP is ch x time
        ...
        %CalcCSD -- 
        CSD = calcCSD(EVP'); %CalCSD needs to be done on the individual trial data
        ...
        for ch = 1:chanN
            ...
            for trialNum = 1:size(SDF,3)
                clear x n Spec
                lfp_holder        = SDF(ch,:,trialNum)'; %lfpHolder comes out in time x 1 units
